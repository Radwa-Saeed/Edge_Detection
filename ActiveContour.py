...
class Active_Contour:
    """ An Active_Contour class for active contour segmentation """
    ...
    SEARCH_KERNEL_SIZE = 9              # The size of the search kernel.
    # Members
    ...
    closed = True       # Indicates if the snake is closed.
    ...
    def __init__( self, image):
        ...
        # Image variations used by the active contour
        self.gray = cv2.cvtColor( self.image, cv2.COLOR_RGB2GRAY )
        self.binary = cv2.adaptiveThreshold( self.gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2 )
        # self.gradientX = sobel_filter(self.gray, kernel_size = 5, direction = "x")
        # self.gradientY = sobel_filter(self.gray, kernel_size = 5, direction = "y")
        self.gradientX = cv2.Sobel( self.gray, cv2.CV_64F, 1, 0, ksize=5 )
        self.gradientY = cv2.Sobel( self.gray, cv2.CV_64F, 0, 1, ksize=5 )

        # Gets the half width and height of the image to use as center of the generated snake circle
    ...
    def dist( a, b ):
        """
        Calculates the length of a line segment between the two points
    ...
    def get_length(self):
        ...
        return np.sum( [ Active_Contour.dist( self.points[i], self.points[ (i+1)%n_points  ] ) for i in range( 0, n_points ) ] )
   

    def fun_uniformity( self, p, prev ):
        """
        ...
        dist = Active_Contour.dist( prev, p )

        uni_dist = abs( dist - avg_dist )

        return uni_dist**2



    def fun_curvature( self, p, prev, next ):
        """
        ...
        cx = float( vx + ux )  / ( ut * vn )
        cy = float( vy + uy ) / ( ut * vn )
    ...
    def fun_line( self, p ):
        ...
        """
        # If the point is out of the bounds of the image, return a high value (since it is a minimization problem)
    ...
    # def fun_term( self, p, prev, next ):
    #     """
    #     The tendency to move the snake towards corners and terminations.
    #     :param p: The point being analysed
    ...
    # def fun_config( self, point , prev, next ):
    #     """
    ...
    def dismiss_overlaping_points( self ):
        """
        dismiss overlaping points from the curve based on
        ...
        for i in range( 0, snake_size ):
            for j in range( snake_size-1, i+1, -1 ):
                ...
                dist = Active_Contour.dist( curr, end )
    ...
    def add_missing_points( self ):
        ...
        for i in range( 0, snake_size ):
            ...
            if Active_Contour.dist( curr, next ) > self.MAX_DISTANCE_BETWEEN_POINTS:
    ...
    def step( self ):
        ...
        new_contour= self.points.copy()


        # Kernels (They store the energy for each point being search along the search kernel)
        search_kernel = ( self.SEARCH_KERNEL_SIZE, self.SEARCH_KERNEL_SIZE )
        half_kernel_size = math.floor( self.SEARCH_KERNEL_SIZE / 2 ) # half-kernel size
        e_uniformity = np.zeros( search_kernel )
        e_curvature = np.zeros( search_kernel )
        e_line = np.zeros( search_kernel )
        e_edge = np.zeros( search_kernel )
        # e_term = np.zeros( search_kernel )
        # e_conf = np.zeros( search_kernel )
        ...
        for i in range( 0, len( self.points ) ):
            ...
            for dx in range( -half_kernel_size, half_kernel_size ):
                for dy in range( -half_kernel_size, half_kernel_size ):
                    point = np.array( [curr[0] + dx, curr[1] + dy] )

                    # Calculates the energy functions on p
                    e_uniformity[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_uniformity( point, prev )
                    e_curvature[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_curvature( point, prev, next )
                    e_line[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_line( point )
                    e_edge[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_edge( point )
                    # e_term[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_term( point, prev, next )
                    # e_conf[ dx + half_kernel_size ][ dy + half_kernel_size ] = self.fun_config( point, prev, next )


            # Normalizes energies
            e_uniformity = Active_Contour.normalize( e_uniformity )
            e_curvature = Active_Contour.normalize( e_curvature )
            e_line = Active_Contour.normalize( e_line )
            e_edge = Active_Contour.normalize( e_edge )
            # e_term = Active_Contour.normalize( e_term )
            # e_conf = Active_Contour.normalize( e_conf )
            ...
            for dx in range( -half_kernel_size, half_kernel_size ):
                for dy in range( -half_kernel_size, half_kernel_size ):
                    if e_sum[ dx + half_kernel_size ][ dy + half_kernel_size ] < emin:
                        emin = e_sum[ dx + half_kernel_size ][ dy + half_kernel_size ]
                        x = curr[0] + dx
                        y = curr[1] + dy
            ...
            new_contour[i] = np.array( [ x, y ] )

        self.points = new_contour

        # Post threatment to the snake, dismiss overlaping points and
        # add missing points
        self.dismiss_overlaping_points()
